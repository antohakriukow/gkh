...
> = memo(({ componentData }) => {
	...
	const toggleGroupVisibility = useCallback((partnerName: string) => {
		setGroupsVisibility(prevState => ({
			...prevState,
			[partnerName]: !prevState[partnerName]
		}))
	}, [])
					...
					{isVisible &&
						sortedOperationsArray.map(([partnerName, operations]) => (
							<OperationsGroup
								...
								isVisible={groupsVisibility[partnerName] ?? false}
								toggleVisibility={() => toggleGroupVisibility(partnerName)}
							/>
						))}
				</div>
				<div></div>
